...
function getGeminiClient(): GoogleGenAI {
  ...
  if (!aiClient) {
    try {
      ...
      throw new Error("Failed to initialize Gemini API client. Check your API key.");
    }
  }
  return aiClient;
}
...
app.post('/api/stress-test', async (req, res) => {
  ...
  try {
    console.log(`Fetching live climate data from Gemini API with Google Search Grounding for [${address}]...`);
...

{
  ...
  "projections": [
    ...
    parsedReport.baselinePriceNote = 'Property price verified with live Google Search grounding and local market data.';

    console.log(`Successfully generated dynamic climate reports for [${address}] via Gemini with live Google Search Grounding.`);
    res.json({ report: parsedReport, source: 'AI_GENERATED', dataQuality: 'VERIFIED_WITH_GROUNDING' });
 ...
 */
async function geocodeAddress(address: string): Promise<{ lat: number, lon: number, name: string }> {
  ...
  if (match) {
    return {
      ...
      name: address.replace(coordRegex, '').replace(/[\[\]]/g, '').trim() || 'Custom Location'
    };
  }
  ...
  if (owmKey && owmKey !== "MY_OPENWEATHERMAP_API_KEY") {
    try {
      ...
      console.error("OpenWeatherMap geocoding failed, falling back to Open-Meteo:", e);
    }
  }
  ...
  if (norm.includes("miami")) {
    return { lat: 25.7907, lon: -80.1300, name: "Miami Beach, Coastal Florida" };
  } else if (norm.includes("phoenix") || norm.includes("desert")) {
    return { lat: 33.4484, lon: -112.0740, name: "Desert Foothills, Phoenix, AZ" };
  ...
  return { lat: 40.7128, lon: -74.0060, name: address };
}
...
app.get('/api/weather', async (req, res) => {
  ...
  try {
    ...
    if (lat && lon) {
      ...
      finalName = (location as string) || `Custom Coordinate [${finalLat.toFixed(4)}, ${finalLon.toFixed(4)}]`;
    } else if (location) {
      ...
      res.status(400).json({ error: "Missing 'location' or 'lat' and 'lon' parameters." });
      return;
    }
    ...
    if (owmKey && owmKey !== "MY_OPENWEATHERMAP_API_KEY") {
      try {
        console.log(`Querying real-time weather from OpenWeatherMap for [${finalLat}, ${finalLon}]`);
        ...
        if (data && data.main) {
          res.json({
            ...
            current: {
              ...
              description: data.weather && data.weather[0] ? data.weather[0].description : 'clear'
            }
          });
          return;
        }
    ...
    console.log(`Querying real-time weather from Open-Meteo for [${finalLat}, ${finalLon}]`);
    ...
    if (data && data.current) {
      res.json({
        ...
        current: {
          ...
          description: (data.current.precipitation > 2 ? 'rainy' : data.current.precipitation > 0 ? 'light drizzle' : 'clear')
        }
      });
    } else {
    ...
    res.status(500).json({ error: `Weather service lookup failed: ${error.message}` });
  }
});
...
app.get('/api/weather/historical', async (req, res) => {
  ...
  try {
    ...
    console.log(`Querying decadal climate history from Open-Meteo Archive for [${finalLat}, ${finalLon}]`);
    ...
    const yearlyData: Record<number, { tempSum: number, tempCount: number, summerTempSum: number, summerTempCount: number, precipSum: number }> = {};
    ...
    for (const yr of sortedYears) {
      ...
      const summerAvg = yrData.summerTempCount > 0 
    ...
    res.status(500).json({ error: `Climate history retrieval failed: ${error.message}` });
  }
});
...
async function initializeServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Sovereign Wealth Board backend active on http://0.0.0.0:${PORT}`);
  });
}
